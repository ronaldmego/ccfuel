...
- **Keep it lightweight.** ccfuel is intentionally a zero-build app with two runtime dependencies: Express, and `node-pty` because there is no non-interactive way to read `/usage`. Please don't introduce a framework, a build step or a third dependency without discussing it first.
