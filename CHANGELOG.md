...
### Fixed
- **A failed or in-flight fetch reported itself as `Error: Unknown`, and the gauges sat at `Loading…` forever.** The reason the fetcher produces was never reaching the screen. The dashboard read `data.error`, a field only the 500 path sets, while the payload carries `errorMessage`; it chose its render branch on the existence of `session` and `weekAll`, two objects a failed fetch still ships with `percent: null` inside — enough to paint a literal `null%`; and the concurrency branch spread an empty cache, so the first fetch after a restart, which takes ~6s and is not a failure at all, came back as a body with neither numbers nor a reason. `/api/global-usage` now answers `fetching` + `failureKind: 'fetch-in-flight'` while a fetch is running, and marks a cached value served after a failed fetch as `stale` with that attempt's `failureKind`/`errorMessage` instead of passing it off as fresh — keeping the last good number is still right, a 0% would be worse. The dashboard renders each state as what it is: `Fetching from Claude…`, `Last good: <time> — <reason>`, or the actionable message with the gauges reading `No reading` ([#49](https://github.com/ronaldmego/ccfuel/issues/49))
- **`npm run demo` could destroy real snapshots.** It resolved its fixture directory as `DASHBOARD_DATA_DIR || ./demo-data`, and the fixture builder overwrites a fixed set of filenames — so anyone who had `DASHBOARD_DATA_DIR` exported for their real dashboard, which is exactly what the README documents, lost `weekly-history.json`, `usage-curve.json`, `session-metrics.json`, `resets-cache.json` and `global-usage.json` the first time they ran the demo. Reproduced with a sentinel file before the fix. The demo now creates a fresh `mkdtemp` directory per run under the OS temp dir, **ignores `DASHBOARD_DATA_DIR` entirely** (and says so), and removes the fixture on normal exit, SIGINT, SIGTERM, a failed spawn and an uncaught error, without orphaning the server. There is no override: looking at the UI does not need write access to real data ([#47](https://github.com/ronaldmego/ccfuel/issues/47))
- `test/demo-entrypoint.test.js` runs `scripts/demo.js` as a child process with `DASHBOARD_DATA_DIR` pointed at a directory full of sentinels and asserts they come back byte-identical, that the fixture lives elsewhere under the temp dir, that no PTY fetch or transcript scan happened, and that the fixture is gone and the port released after each signal. The existing server smoke could not have caught this: it calls `buildFixture()` directly, so it never exercised the entrypoint that chose the directory — which is where the whole bug lived ([#47](https://github.com/ronaldmego/ccfuel/issues/47))
